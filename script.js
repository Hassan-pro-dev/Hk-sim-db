    ...
    if (modal && followBtn) {
        ...
        function checkCode() {
            ...
            if (code === 'Funki') {
                ...
                inputLine.style.boxShadow = '0 0 10px red';
                
                setTimeout(() => {
                    inputLine.style.borderColor = ''; // Reset to default (CSS)
                    inputLine.style.boxShadow = '';
                }, 1000);
            }
        }
...
function fetchData() {
    ...
    if (isLocalhost) {
        // Use corsproxy.io for local development
        const targetUrl = `https://amscript.xyz/PublicApi/Siminfo.php?number=${numberInput}`;
        apiUrl = `https://corsproxy.io/?${encodeURIComponent(targetUrl)}`;
    } else {
        // Use Netlify internal proxy (defined in _redirects) for production
        apiUrl = `/api/Siminfo.php?number=${numberInput}`;
    }
